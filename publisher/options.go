...
// ManagementOption provides structure for configuring a new TopicPublisher
...
func SetDefaultHeader(headerName, msgKey string) ManagementOption {
	return func(p Publisher) error {
		return p.SetDefaultHeader(headerName, msgKey)
	}
}
...
func WithDuplicateDetection(window *time.Duration) ManagementOption {
	return func(p Publisher) error {
		if tp, ok := p.(*TopicPublisher); ok {
			tp.topicManagementOptions = append(tp.topicManagementOptions, servicebus.TopicWithDuplicateDetection(window))
			return nil
		}
		return errors.New("given publisher does not support DuplicateDetection")
	}
}
